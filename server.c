...
Client *client_new(int id) {
	...
	pthread_t *thread_for_watchdog = (pthread_t *)malloc(sizeof(pthread_t));
	...
	Timeout *new_timeout = timeout_new(wait_s, pthread_deletion_wrapper, (void *) thread_for_run);
	client->my_timeout = new_timeout;
	
	//creating the thread for watchdog
	pthread_create(thread_for_watchdog, 0, watchdog_function, (void *) new_timeout);
	pthread_detach(*thread_for_watchdog);
	client->watchdog_pthread = thread_for_watchdog;
...
void client_delete(Client *client) {
	...
	timeout_delete(client->my_timeout);
	...
	pthread_t new_pthread = *client->watchdog_pthread;
	free(client);
	pthread_cancel(new_pthread);
...
void *run_client(void *client) {
	...
	timeout_activate(((Client *) client)->my_timeout);
	...
	while(get_command(((Client *) client)->window, command)) {
		...
		//first of all: deactivate
		timeout_deactivate(((Client *) client)->my_timeout);
